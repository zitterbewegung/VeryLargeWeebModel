...
We generate training data from Tokyo PLATEAU~\cite{plateau}:
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
...
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{Batch size}: 1 per GPU
    \item \textbf{Optimizer}: AdamW, lr=$10^{-4}$, weight decay=0.01
    \item \textbf{Schedule}: Cosine annealing, warmup=100 iterations
    \item \textbf{Epochs}: 50
    \item \textbf{Hardware}: NVIDIA GPU with 24GB+ VRAM
\end{itemize}
...
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{No real sensor data}: Synthetic LiDAR/images
\end{itemize}
